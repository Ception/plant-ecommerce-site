...
export async function POST(req) {
  ...
  const orderLineID = uuid();
    ...
    try {
        const response = await axios.get(`http://localhost:3000/api/getPrices?productID=${productID}&weightID=${weightID}`);
    ...
    try {
      ...
      if (!orderLineResponse) {
        return res.json({ message: 'item could not be added: No response from function.' });
      }
